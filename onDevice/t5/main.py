...
model = T5ForConditionalGeneration.from_pretrained('/new_disk/houyz/gjx_SplitFM/google-t5/t5-base/')
tokenizer = T5Tokenizer.from_pretrained('/new_disk/houyz/gjx_SplitFM/google-t5/t5-base/')
...
#client_datasets = [load_dataset('cais/mmlu',task,cache_dir='/new_disk/houyz/gjx_SplitFM/mmlu_dataset/') for task in mmlu_tasks]
#print("Dataset Loaded Successfully!")
#client_datasets = [
#    load_from_disk(f"/new_disk/houyz/gjx_SplitFM/mmlu_dataset/{task}")
...
with open('./embedding/embedding.pkl','wb') as f:
    pickle.dump(model.shared.weight,f)
...
for client_num in range(num_clients):
    ...
    #print(model)
    #print(tokenized_datasets[client_num]['test'])
    #print(tokenized_datasets[client_num]['test'].shape)
...
for round in range(num_rounds):  # Number of communication rounds
    ...
    for i in range(num_clients):
       # print_memory_usage()
        ...
        #for input_ids, attention_mask in client_dataloader :
